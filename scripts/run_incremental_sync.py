"""
Incremental sync — runs all pipelines respecting existing checkpoints.

Usage:
    python scripts/run_incremental_sync.py [--dry-run] [--workers N]
...
from app.pipelines import IncrementalPipeline, build_pipeline_context


def main() -> None:
    parser = argparse.ArgumentParser(description="Run incremental sync pipeline")
    ...
    print(f"Starting incremental sync  run_id={run_id}  dry_run={args.dry_run}  workers={args.workers}\n")

    ctx = build_pipeline_context(run_id=run_id, dry_run=args.dry_run)
    pipeline = IncrementalPipeline(ctx, max_parallel_workers=args.workers)
